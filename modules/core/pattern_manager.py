...
def log_execution_time(pattern_name: str, table_type: str, speed: int, actual_time: float,
    ...
    Args:
        ...
        table_type: Type of table (e.g., 'kinetiq_motion', 'kinetiq_motion_mini')
...
def is_clear_pattern(file_path):
    ...
    clear_patterns = []
    for table_type in ['kinetiq_motion', 'kinetiq_motion_mini', 'kinetiq_motion_pro']:
        clear_patterns.extend([
            f'./patterns/clear_from_out{("_" + table_type.split("_")[-1]) if table_type != "kinetiq_motion" else ""}.thr',
            f'./patterns/clear_from_in{("_" + table_type.split("_")[-1]) if table_type != "kinetiq_motion" else ""}.thr',
            f'./patterns/clear_sideway{("_" + table_type.split("_")[-1]) if table_type != "kinetiq_motion" else ""}.thr'
        ])
