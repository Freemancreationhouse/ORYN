...
class AppState:
    def __init__(self):
        ...
        # Studio Kinematics universal radial calibration.
        # None => use original source radial scaling unchanged.
        ...
        self.app_name = "KinetiQ Motion"  # Default app name
        ...
        self.table_name = "KinetiQ Motion"  # User-customizable table name
        ...
        self.mqtt_device_id = "kinetiq_motion"  # Device ID for Home Assistant
        self.mqtt_device_name = "KinetiQ Motion"  # Device display name
    ...
    def from_settings_dict(self, data):
        ...
        self.app_name = data.get("app_name", "KinetiQ Motion")
        self.table_id = data.get("table_id", None)
        if self.table_id is None:
            self.table_id = str(uuid.uuid4())
        self.table_name = data.get("table_name", "KinetiQ Motion")
        ...
        # via the UI, so any stored "kinetiq_motion" is a stale default, not an
        # explicit choice. Treat it as unset so the handler generates a unique id.
        stored_client_id = data.get("mqtt_client_id")
        self.mqtt_client_id = None if stored_client_id in (None, "", "kinetiq_motion") else stored_client_id
        self.mqtt_discovery_prefix = data.get("mqtt_discovery_prefix", "homeassistant")
        self.mqtt_device_id = data.get("mqtt_device_id", "kinetiq_motion")
        self.mqtt_device_name = data.get("mqtt_device_name", "KinetiQ Motion")
