...
def connect_device(homing=True):
    ...
    # Note: KinetiQ LEDs are initialized at startup in main.py, so we preserve the existing controller
    if state.led_provider == "wled" and state.wled_ip:
        ...
        # KinetiQ LEDs are already initialized in main.py at startup
...
def get_machine_steps(timeout=10):
    ...
    if settings_complete:
        ...
        if effective_table_type in ['kinetiq_motion_mini', 'kinetiq_motion_mini_pro', 'kinetiq_motion_mini_pro_byj', 'kinetiq_motion_gold']:
