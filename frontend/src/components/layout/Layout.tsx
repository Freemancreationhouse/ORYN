...
import { Grid2X2, ListMusic, SlidersHorizontal, Lightbulb, Settings2, LockKeyhole, Play, Square } from 'lucide-react'

const navItems = [
  { path: '/', label: 'Library', Icon: Grid2X2, title: 'Pattern Library' },
  { path: '/playlists', label: 'Sequences', Icon: ListMusic, title: 'Playlists' },
  { path: '/table-control', label: 'Motion', Icon: SlidersHorizontal, title: 'Table Control' },
  { path: '/led', label: 'Light', Icon: Lightbulb, title: 'LED Control' },
  { path: '/settings', label: 'System', Icon: Settings2, title: 'Settings' },
]

const DEFAULT_APP_NAME = 'ORYN'
...
export function Layout() {
  ...
  const expectedFirmwareVersion = tableType === 'oryn_mini' ? 'v3.8.3' : 'v3.9.5'
  ...
  const handleDownloadLogs = () => {
    ...
    a.href = url
    a.download = `oryn-logs-${new Date().toISOString().split('T')[0]}.txt`
    a.click()
    URL.revokeObjectURL(url)
  }

  const handleRestart = async () => {
    if (!confirm('Are you sure you want to restart ORYN?')) return

    try {
      await apiClient.post('/api/system/restart')
      toast.success('ORYN is restarting...')
    } catch {
      toast.error('Failed to restart ORYN')
    }
  }
    ...
    <div className="min-h-dvh bg-background flex flex-col">
      ...
      <header className="fixed top-0 left-0 right-0 z-40 pt-safe">
        ...
        <div className="relative w-full max-w-5xl mx-auto px-3 sm:px-4 pt-3 pointer-events-none">
          ...
          <div className="flex items-center gap-2">
            <Link to="/">
              <img
                src={customLogo ? apiClient.getAssetUrl(`/static/custom/${customLogo}`) : apiClient.getAssetUrl('/static/custom/studio-kinematics-logo.png')}
                alt={displayName}
                className="w-9 h-9 object-cover rounded-full km-brand-logo"
              />
            </Link>
            <TableSelector>
      ...
      {!isNowPlayingOpen && (
        <button
          ...
          className={`fixed z-[55] flex items-center gap-2 px-4 py-2 rounded-full bg-card border border-border shadow-lg select-none touch-none ${
            isDraggingButton
        ...
        >
          <span className={`km-now-icon ${isCurrentlyPlaying ? 'is-live' : ''}`}>{isCurrentlyPlaying ? <Play size={18} fill="currentColor"/> : <Square size={16}/>}</span>
          <span className="text-sm font-medium">
        ...
        <div className={`max-w-5xl mx-auto grid h-16`} style={{ gridTemplateColumns: `repeat(${visibleNavItems.length + (isPlayOnlyActive ? 1 : 0)}, minmax(0, 1fr))` }}>
          {visibleNavItems.map((item) => {
            ...
            return (
              ...
              >
                <span className={`km-nav-icon ${isActive ? 'is-active' : ''}`}><item.Icon size={20} strokeWidth={isActive ? 2.3 : 1.8}/></span>
                <span className="text-[11px] font-semibold tracking-wide">{item.label}</span>
                {isActive && <span className="km-nav-active-dot" />}
              </Link>
            )
          })}
          ...
          {isPlayOnlyActive && (
            ...
            >
              <span className="km-nav-icon"><LockKeyhole size={20}/></span>
              <span className="text-[11px] font-semibold tracking-wide">System</span>
            </button>
          )}
        </div>
      </nav>
