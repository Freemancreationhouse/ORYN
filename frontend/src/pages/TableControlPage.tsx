...
export function TableControlPage() {
  ...
  const handleStartPerimeterCalibration = async () => {
    ...
    if (Math.abs(currentRho) > 0.05) return toast.error('HOME or move to Center first')
    setCalibrationBusy(true)
    try { await apiClient.post('/api/perimeter-calibration/start'); toast.success('Jog OUT until the ball reaches the exact physical edge') }
    catch (e) { toast.error(e instanceof Error ? e.message : 'Could not start perimeter setup') }
    finally { setCalibrationBusy(false) }
  }
  const handlePerimeterJog = async (units: number) => {
    setCalibrationBusy(true)
    try { await apiClient.post('/api/perimeter-calibration/jog', { units, speed: 60 }) }
    catch (e) { toast.error(e instanceof Error ? e.message : 'Jog failed') }
    finally { setCalibrationBusy(false) }
  }
  const handleSavePerimeter = async () => {
    setCalibrationBusy(true)
    try { const r=await apiClient.post<{rho_travel_units:number}>('/api/perimeter-calibration/save'); setPerimeterUnitsInput(r.rho_travel_units.toFixed(3)); toast.success(`Perimeter saved: ${r.rho_travel_units.toFixed(3)} units`) }
    catch (e) { toast.error(e instanceof Error ? e.message : 'Could not save perimeter') }
    finally { setCalibrationBusy(false) }
  }
  const handleSetPerimeterUnits = async () => {
    const units=Number(perimeterUnitsInput); if(!Number.isFinite(units)||units<=0)return toast.error('Enter a valid controller-unit travel')
    setCalibrationBusy(true)
    try { await apiClient.post('/api/perimeter-calibration/set',{units}); toast.success('Perimeter travel updated') }
    catch(e){toast.error(e instanceof Error?e.message:'Could not update perimeter')}
    finally{setCalibrationBusy(false)}
  }
  const handleResetPerimeterCalibration = async () => {
    setCalibrationBusy(true)
    try { await apiClient.post('/api/perimeter-calibration/reset'); setPerimeterUnitsInput(''); toast.success('Original source radial scale restored') }
    catch { toast.error('Could not reset perimeter calibration') }
    finally { setCalibrationBusy(false) }
  }
    ...
    <TooltipProvider>
      <div className="flex flex-col w-full max-w-5xl mx-auto gap-6 py-3 sm:py-6 px-0 sm:px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {/* Universal Perimeter Calibration */}
          <Card className="transition-all duration-200 hover:shadow-md hover:border-primary/20">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between gap-3">
                <div><CardTitle className="text-lg">Perimeter Calibration</CardTitle><CardDescription>Teach exact Center → Perimeter travel once for this table</CardDescription></div>
                <Badge variant={rhoCalibrated ? 'default' : 'secondary'}>{rhoCalibrated ? 'CALIBRATED' : 'SOURCE DEFAULT'}</Badge>
              </div>
            </CardHeader>
            <CardContent className="space-y-4">
              <Alert><span className="material-icons-outlined text-base mr-2 shrink-0">straighten</span><AlertDescription className="text-xs">No physical radius in mm is required. HOME is rho=0; the saved physical edge is rho=1. Original theta, gear and coupling mathematics remain intact.</AlertDescription></Alert>
              {!perimeterCalibrationActive ? <div className="space-y-3">
                <Button onClick={handleStartPerimeterCalibration} disabled={calibrationBusy||isHoming||isPatternRunning||Math.abs(currentRho)>0.05} className="w-full gap-2"><span className="material-icons-outlined">my_location</span>Start From Current Center</Button>
                <div className="flex gap-2"><Input value={perimeterUnitsInput} onChange={(e)=>setPerimeterUnitsInput(e.target.value)} type="number" step="0.001" min="0.001" placeholder="Saved controller units"/><Button variant="secondary" onClick={handleSetPerimeterUnits} disabled={calibrationBusy||!perimeterUnitsInput}>Update</Button></div>
                <div className="flex items-center justify-between text-xs text-muted-foreground"><span>Saved travel</span><span className="font-mono">{rhoCalibrated&&rhoTravelUnits!==null?`${rhoTravelUnits.toFixed(3)} units`:'Original source geometry'}</span></div>
                {rhoCalibrated&&<Button variant="outline" size="sm" onClick={handleResetPerimeterCalibration} disabled={calibrationBusy}>Reset to Source Default</Button>}
              </div> : <div className="space-y-3">
                <div className="rounded-lg border bg-muted/30 p-3 flex items-center justify-between"><span className="text-sm font-medium">Center → Edge travel</span><span className="font-mono font-semibold">{perimeterCalibrationCurrentUnits.toFixed(3)} units</span></div>
                <div className="grid grid-cols-3 gap-2"><Button variant="secondary" onClick={()=>handlePerimeterJog(-0.2)} disabled={calibrationBusy}>IN 0.2</Button><Button variant="secondary" onClick={()=>handlePerimeterJog(0.2)} disabled={calibrationBusy}>OUT 0.2</Button><Button variant="secondary" onClick={()=>handlePerimeterJog(1)} disabled={calibrationBusy}>OUT 1</Button><Button variant="secondary" onClick={()=>handlePerimeterJog(-1)} disabled={calibrationBusy}>IN 1</Button><Button variant="secondary" onClick={()=>handlePerimeterJog(2)} disabled={calibrationBusy}>OUT 2</Button><Button variant="secondary" onClick={()=>handlePerimeterJog(5)} disabled={calibrationBusy}>OUT 5</Button></div>
                <Button onClick={handleSavePerimeter} disabled={calibrationBusy||perimeterCalibrationCurrentUnits<1} className="w-full gap-2"><span className="material-icons-outlined">save</span>Save This Physical Position as Perimeter</Button>
              </div>}
            </CardContent>
          </Card>
