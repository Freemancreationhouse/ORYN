...
export function BrowsePage() {
  ...
  return (
    <div className="flex flex-col w-full max-w-5xl mx-auto gap-3 sm:gap-6 py-3 sm:py-6 px-0 sm:px-4">
      ...
      <div className="flex items-start justify-between gap-4 pl-1">
        ...
        {!isPlayOnlyActive && (
          <div className="flex items-center gap-2">
            <Button variant="outline" onClick={() => fileInputRef.current?.click()} disabled={isUploading} className="km-toolbar-button gap-2">
              <Upload size={17}/><span className="hidden sm:inline">Import THR</span>
            </Button>
            <Button onClick={() => setIsGeneratorOpen(true)} className="km-forge-button gap-2">
              <WandSparkles size={18}/><span>Pattern Forge</span>
            </Button>
          </div>
        )}
      </div>
      ...
      <PatternGeneratorDialog open={isGeneratorOpen} onOpenChange={setIsGeneratorOpen} onSaved={fetchPatterns} />
        ...
        >
          ...
          {selectedPattern && (
            <div className="p-6 overflow-y-auto flex-1">
              ...
              <div className="mb-6">
                <div
                  className="aspect-square w-full max-w-[300px] mx-auto overflow-hidden km-preview-frame relative group cursor-pointer"
                ...
                >
                  ...
                  <div className="absolute bottom-3 right-3 km-preview-play group-hover:scale-110 transition-transform">
              ...
              {/* ORYN clearing mode selector */}
              <div className="mb-7">
                <div className="flex items-center justify-between mb-3"><div><p className="km-eyebrow">PRE-RUN SURFACE</p><Label className="text-base font-semibold">Clearing strategy</Label></div><span className="km-mini-badge">choose before play</span></div>
                <div className="km-clear-grid">
                  {preExecutionOptions.map((option) => {
                    const Icon = option.value === 'adaptive' ? Sparkles : option.value === 'clear_from_in' ? Crosshair : option.value === 'clear_from_out' ? CircleDot : option.value === 'clear_sideway' ? MoveDiagonal2 : SkipForward
                    return <button key={option.value} type="button" onClick={() => setPreExecution(option.value)} className={`km-clear-tile ${preExecution === option.value ? 'is-selected' : ''}`}>
                      <span className="km-clear-icon"><Icon size={22}/></span>
                      <span className="km-clear-copy"><b>{option.label === 'Sideways' ? 'Diagonal' : option.label}</b><small>{option.value === 'adaptive' ? 'Ball-position smart clear' : option.value === 'clear_from_in' ? 'Center → edge' : option.value === 'clear_from_out' ? 'Edge → center' : option.value === 'clear_sideway' ? 'Side/diagonal sweep' : 'Draw immediately'}</small></span>
                    </button>
                  })}
                </div>
                <p className="text-xs text-muted-foreground mt-3">{preExecutionOptions.find(o => o.value === preExecution)?.description}</p>
              </div>

              {/* ORYN action deck */}
              <div className="km-action-deck">
                <div className="flex items-stretch gap-3">
                  <Button onClick={handleRunPattern} disabled={isRunning} className="km-play-command flex-1" size="lg">
                    <span className="km-play-symbol">{isRunning ? <span className="material-icons-outlined animate-spin">sync</span> : <Play size={24} fill="currentColor"/>}</span>
                    <span><b>{isRunning ? 'Starting…' : 'Run Pattern'}</b><small>execute selected route</small></span>
                  </Button>
                  {canDelete && <Button variant="outline" onClick={handleDeletePattern} className="km-square-danger" size="lg"><Trash2 size={20}/></Button>}
                </div>
                <div className="grid grid-cols-2 gap-3 mt-3">
                  <Button variant="outline" className="km-queue-command" onClick={() => handleAddToQueue('next')}><ListStart size={19}/><span><b>Next</b><small>after current</small></span></Button>
                  <Button variant="outline" className="km-queue-command" onClick={() => handleAddToQueue('end')}><ListPlus size={19}/><span><b>Queue</b><small>add to end</small></span></Button>
                </div>
              </div>
            </div>
          )}
