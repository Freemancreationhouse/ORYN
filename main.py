...
async def lifespan(app: FastAPI):
    # Startup
    logger.info("Starting ORYN — Designed to Move...")
            ...
            if state.conn and state.conn.is_connected():
                if not state.home_on_connect:
                    ...
                    try:
                        ...
                        if not success:
                            logger.warning("Background homing did not complete")
                            if getattr(state, "homing_error", None) == "perimeter_calibration_required":
                                logger.warning("Fresh perimeter calibration is required before sensorless HOME; leaving controller connected for setup")
                        ...
                        logger.info("Background homing cycle ended")
    ...
    try:
        ...
        if state.led_provider == "wled" and state.wled_ip:
            ...
            logger.info(f"LED controller initialized: KinetiQ LEDs ({state.dw_led_num_leds} LEDs on GPIO{state.dw_led_gpio_pin}, pixel order: {state.dw_led_pixel_order})")
            ...
            if status.get("connected", False):
                if state.led_automation_enabled:
                    ...
                    logger.info("KinetiQ LEDs hardware initialized and idle effect started")
                else:
                    logger.info("KinetiQ LEDs hardware initialized (manual mode, no auto-effect)")
            else:
                error_msg = status.get("error", "Unknown error")
                logger.warning(f"KinetiQ LED hardware initialization failed: {error_msg}")
        else:
        ...
        while True:
            try:
                ...
                if idle_seconds >= timeout_seconds:
                    status = state.led_controller.check_status()
                    # Check both "power" (WLED) and "power_on" (KinetiQ LEDs) keys
    ...
    # Shutdown
    logger.info("Shutting down ORYN...")

app = FastAPI(lifespan=lifespan)

# Add CORS middleware to allow cross-origin requests from other ORYN frontends
...
# ORYN production React assets generated by Vite use absolute /assets/* URLs.
# Serve the actual compiled assets instead of letting the SPA fallback return
# index.html for JavaScript/CSS requests.
_FRONTEND_DIST_DIR = Path("static") / "dist"
_FRONTEND_ASSETS_DIR = _FRONTEND_DIST_DIR / "assets"
if _FRONTEND_ASSETS_DIR.exists():
    app.mount("/assets", StaticFiles(directory=str(_FRONTEND_ASSETS_DIR)), name="oryn-assets")
...
# ORYN packaged/web frontend
# Serve the production React build directly. This avoids the legacy Jinja
# redirect template and allows Windows/Pi packaged installations to use the
# same UI from port 8080.
_FRONTEND_DIST = Path("static") / "dist"
...
def get_frontend_response():
    ...
    pd_tag = '<script defer src="/static/custom/oryn-pattern-designer-integration.js?v=PD-PI-V1-20260831-1"></script>'
    ...
    return HTMLResponse(
        ...
        headers={
            ...
            "Pragma": "no-cache",
            "Expires": "0",
            "X-ORYN-Universal-Calibration": "UC-ACTUAL-SERVE-20260826-1",
        },
    )
...
async def pattern_designer_page():
    ...
    # Keep the designer self-contained under /static while presenting a clean
    # first-class ORYN route at /pattern-designer.
    ...
    return HTMLResponse(
        ...
        headers={
            ...
            "Pragma": "no-cache",
            "Expires": "0",
            "X-ORYN-Pattern-Designer": "PD-PI-V1-20260831-1",
        },
    )

@app.get("/registerSW.js", include_in_schema=False)
async def oryn_register_service_worker():
    sw_register = _FRONTEND_DIST / "registerSW.js"
    if not sw_register.exists():
        raise HTTPException(status_code=404, detail="Service worker registration script not found")
    return FileResponse(sw_register)

@app.get("/sw.js", include_in_schema=False)
async def oryn_service_worker():
    service_worker = _FRONTEND_DIST / "sw.js"
    if not service_worker.exists():
        raise HTTPException(status_code=404, detail="Service worker not found")
    return FileResponse(service_worker)
    ...
    return {
        ...
        "machine": {
            ...
            "available_table_types": [
                {"value": "kinetiq_motion_mini", "label": "KinetiQ Mini / Compatible"},
                {"value": "kinetiq_motion_mini_pro", "label": "KinetiQ Mini Pro / Compatible"},
                {"value": "kinetiq_motion_mini_pro_byj", "label": "KinetiQ Mini Pro BYJ / Compatible"},
                {"value": "kinetiq_motion_gold", "label": "KinetiQ Gold / Compatible"},
                {"value": "kinetiq_motion", "label": "KinetiQ Standard / Compatible"},
                {"value": "kinetiq_motion_pro", "label": "KinetiQ Pro / Compatible"}
            ]
        },
        "security": {
...
async def get_dynamic_manifest():
    ...
    app_name = state.app_name or "KinetiQ Motion"
...
async def update_settings(settings_update: SettingsUpdate):
    ...
    Example: {"app": {"name": "KinetiQ Motion"}, "auto_play": {"enabled": true}}
    """
    updated_categories = []
    ...
    if settings_update.app:
        if settings_update.app.name is not None:
            state.app_name = settings_update.app.name or "KinetiQ Motion"
    ...
    if settings_update.machine:
        ...
        if m.gear_ratio_override is not None:
            ...
            if state.gear_ratio_override is not None:
                ...
                effective_table_type = state.table_type_override or state.table_type
                mini_types = ['kinetiq_motion_mini', 'kinetiq_motion_mini_pro', 'kinetiq_motion_mini_pro_byj', 'kinetiq_motion_gold']
...
async def update_table_info(update: TableInfoUpdate):
    ...
    if update.name is not None:
        state.table_name = update.name.strip() or "KinetiQ Motion"
        state.save()
...
async def debug_serial_open(request: DebugSerialRequest):
    """Open terminal; reuse the active controller connection when it owns this port."""
    import serial
    async with get_debug_serial_lock():
        if state.conn and state.conn.is_connected() and request.port == state.port:
            _debug_serial_connections[request.port] = "__MAIN_CONNECTION__"
            logger.info(f"Terminal attached to active controller {request.port}")
            return {"success":True,"port":request.port,"baudrate":request.baudrate,"shared":True}
        old=_debug_serial_connections.pop(request.port,None)
        if old and old != "__MAIN_CONNECTION__":
            try: old.close()
            except Exception: pass
        try:
            ser=serial.Serial(request.port,baudrate=request.baudrate,timeout=request.timeout)
            _debug_serial_connections[request.port]=ser
            return {"success":True,"port":request.port,"baudrate":request.baudrate,"shared":False}
        except Exception as e:
            raise HTTPException(status_code=500,detail=str(e))

@app.post("/api/debug-serial/close", tags=["debug-serial"])
async def debug_serial_close(request: ConnectRequest):
    """Detach terminal without disconnecting the main controller."""
    async with get_debug_serial_lock():
        target=_debug_serial_connections.pop(request.port,None)
        if target is None: return {"success":True,"message":"Port not open"}
        if target == "__MAIN_CONNECTION__": return {"success":True,"shared":True}
        try: target.close()
...
async def debug_serial_send(request: DebugSerialCommand):
    """Real raw controller terminal using the active machine connection when shared."""
    async with get_debug_serial_lock():
        if request.port not in _debug_serial_connections:
            raise HTTPException(status_code=400,detail="Port not open. Open it first.")
        target=_debug_serial_connections[request.port]
        cmd=request.command.strip()
        if not cmd: return {"success":True,"command":"","responses":[],"raw":""}
        if target == "__MAIN_CONNECTION__":
            if state.is_homing or state.current_playing_file:
                raise HTTPException(status_code=409,detail="Terminal is locked while HOME or a pattern is running")
            if not (state.conn and state.conn.is_connected()):
                raise HTTPException(status_code=400,detail="Main controller disconnected")
            conn=state.conn
            try:
                ...
                while time.time()-started < request.timeout:
                    line=await asyncio.to_thread(conn.readline)
                    if line:
...
async def debug_serial_status():
    ...
    async with get_debug_serial_lock():
        ...
        for port, ser in _debug_serial_connections.items():
            if ser == "__MAIN_CONNECTION__":
                status[port] = {"open": bool(state.conn and state.conn.is_connected()), "baudrate": 115200, "shared": True}
            else:
                try: status[port] = {"open": ser.is_open, "baudrate": ser.baudrate, "shared": False}
                except Exception: status[port] = {"open": False, "shared": False}
...
class PatternGeneratorSaveRequest(BaseModel):
    token: str
    name: str
...
@app.post("/api/v2/pattern-generator/save")
async def pattern_generator_save(request: PatternGeneratorSaveRequest):
    token=re.sub(r'[^a-fA-F0-9]','',request.token)
    temp_thr=os.path.join(_PATTERN_GENERATOR_TMP,token+".thr")
    if not token or not os.path.isfile(temp_thr):
        raise HTTPException(status_code=404,detail="Generated preview expired. Generate it again.")
    safe_name=re.sub(r'[^A-Za-z0-9 _.-]+','_',request.name or '').strip(' ._')
    if not safe_name: raise HTTPException(status_code=400,detail="Enter a pattern name")
    if not safe_name.lower().endswith('.thr'): safe_name += '.thr'
    custom_dir=os.path.join(pattern_manager.THETA_RHO_DIR,"custom_patterns")
    os.makedirs(custom_dir,exist_ok=True)
    dest=os.path.join(custom_dir,safe_name)
    if os.path.exists(dest): raise HTTPException(status_code=409,detail="A pattern with this name already exists")
    shutil.copyfile(temp_thr,dest)
    rel=f"custom_patterns/{safe_name}"
    try: await generate_image_preview(rel)
...
# ============================================================================
# ORYN Pattern Designer — Pi library bridge
# ============================================================================
# Additive integration only. This endpoint writes a generated THR file into
# the existing custom pattern library and generates the normal library preview.
# It does not call, alter, or participate in machine motion/calibration.
@app.get("/api/pattern-designer/status", tags=["pattern-designer"])
async def pattern_designer_status():
    return {
        "enabled": True,
        "version": "2.0-pro-77",
        "integration": "PD-PI-V1-20260831-1",
...
async def api_v2_capabilities():
    return {"api":"v2","product":"KinetiQ Motion","pattern_generator":{"enabled":True,"formats":["svg","dxf","png","jpg","jpeg","webp","bmp","gcode","nc","ngc","tap","thr"]},"motion_core":"preserved-source-core"}
...
def _restore_machine_calibration_backup():
    """Restore only the NEW v3 profile.

    Legacy/v2 calibration is intentionally invalidated once for this build, per
    ...
    try:
        ...
        if os.path.exists(CALIBRATION_BACKUP_FILE):
            with open(CALIBRATION_BACKUP_FILE, "r", encoding="utf-8") as fh:
                data = json.load(fh)
        if int(data.get("profile_version", 0) or 0) != 3:
            logger.warning("Invalidating previous machine calibration; fresh v3 calibration required")
        ...
        if changed:
            state.save()
            logger.info("Restored universal v3 machine profile from persistent backup")
    except Exception as exc:
...
class PerimeterCalibrationSetRequest(BaseModel):
    units: float

@app.get("/api/perimeter-calibration", tags=["machine-calibration"])
async def get_perimeter_calibration():
    original_units=100.0/(3.7 if state.table_type=='kinetiq_motion_mini' else 5.0)
    effective=state.rho_travel_units if state.rho_calibrated and state.rho_travel_units else original_units
    return {"calibrated":bool(state.rho_calibrated and state.rho_travel_units),"rho_travel_units":state.rho_travel_units,"effective_units":effective,"original_source_units":original_units,"active":state.perimeter_calibration_active,"current_units":state.perimeter_calibration_current_units}

@app.post("/api/perimeter-calibration/start", tags=["machine-calibration"])
async def start_perimeter_calibration():
    if not (state.conn and state.conn.is_connected()): raise HTTPException(status_code=400,detail="Connection not established")
    check_homing_in_progress()
    if state.current_playing_file: raise HTTPException(status_code=409,detail="Stop the current pattern first")
    if abs(float(state.current_rho))>0.05: raise HTTPException(status_code=409,detail="HOME or move to Center first")
    try: await connection_manager.update_machine_position()
    except Exception as e: logger.warning(f"Calibration start position refresh failed: {e}")
    state.perimeter_calibration_start_y=float(state.machine_y)
    state.perimeter_calibration_current_units=0.0
    state.perimeter_calibration_active=True
    return {"success":True,"start_y":state.perimeter_calibration_start_y}

@app.post("/api/perimeter-calibration/jog", tags=["machine-calibration"])
async def jog_perimeter_calibration(request: PerimeterCalibrationJogRequest):
    ...
    if not (state.conn and state.conn.is_connected()): raise HTTPException(status_code=400,detail="Connection not established")
    units=float(request.units)
    if abs(units)>10: raise HTTPException(status_code=400,detail="Jog limited to 10 controller units per click")
    speed=max(5.0,min(float(request.speed),200.0))
    before=float(state.machine_y)
    ok=await connection_manager.send_grbl_coordinates(0,units,speed,timeout=30,home=True)
    if not ok: raise HTTPException(status_code=500,detail="Controller did not acknowledge calibration jog")
    await connection_manager.check_idle_async(timeout=60)
    try: await connection_manager.update_machine_position()
    except Exception: state.machine_y=before+units
    state.perimeter_calibration_current_units=abs(float(state.machine_y)-float(state.perimeter_calibration_start_y))
    return {"success":True,"current_units":state.perimeter_calibration_current_units,"machine_y":state.machine_y}

@app.post("/api/perimeter-calibration/save", tags=["machine-calibration"])
async def save_perimeter_calibration():
    if not state.perimeter_calibration_active or state.perimeter_calibration_start_y is None: raise HTTPException(status_code=409,detail="Start perimeter calibration first")
    try: await connection_manager.update_machine_position()
    except Exception: pass
...
async def cancel_perimeter_calibration():
    state.perimeter_calibration_active=False; state.perimeter_calibration_start_y=None; state.perimeter_calibration_current_units=0.0
    return {"success":True}
...
async def set_led_config(request: LEDConfigRequest):
    """DEPRECATED: Use PATCH /api/settings instead. Configure LED provider (WLED, KinetiQ LEDs, or none)"""
    ...
    if request.provider == "wled":
        ...
        # Stop existing KinetiQ LED controller if hardware settings changed
        ...
        logger.info(f"KinetiQ LEDs configured{restart_msg}: {state.dw_led_num_leds} LEDs on GPIO{state.dw_led_gpio_pin}, pixel order: {state.dw_led_pixel_order}")

        # Check if initialization succeeded by checking status
        status = state.led_controller.check_status()
        if not status.get("connected", False) and status.get("error"):
            error_msg = status["error"]
            logger.warning(f"KinetiQ LED initialization failed: {error_msg}, but configuration saved for testing")
            state.led_controller = None
...
async def set_app_name(request: dict):
    ...
    if not app_name:
        app_name = "KinetiQ Motion"  # Reset to default if empty
...
async def set_mqtt_config(request: dict):
    ...
    try:
        ...
        state.mqtt_device_id = (request.get("device_id") or "kinetiq_motion").strip()
        state.mqtt_device_name = (request.get("device_name") or "KinetiQ Motion").strip()
...
async def test_mqtt_connection(request: dict):
    ...
    client_id = (request.get("client_id") or "kinetiq_motion_test").strip()
...
# KinetiQ LED control endpoints
@app.get("/api/dw_leds/status")
async def dw_leds_status():
    """Get KinetiQ LED controller status"""
    if not state.led_controller or state.led_provider != "dw_leds":
        return {"connected": False, "message": "KinetiQ LEDs not configured"}

    try:
        return state.led_controller.check_status()
    except Exception as e:
        logger.error(f"Failed to check KinetiQ LED status: {str(e)}")
        return {"connected": False, "message": str(e)}

@app.post("/api/dw_leds/power")
async def dw_leds_power(request: dict):
    """Control KinetiQ LED power (0=off, 1=on, 2=toggle)"""
    if not state.led_controller or state.led_provider != "dw_leds":
        raise HTTPException(status_code=400, detail="KinetiQ LEDs not configured")
    ...
    try:
        ...
        logger.error(f"Failed to set KinetiQ LED power: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/api/dw_leds/brightness")
async def dw_leds_brightness(request: dict):
    """Set KinetiQ LED brightness (0-100)"""
    if not state.led_controller or state.led_provider != "dw_leds":
        raise HTTPException(status_code=400, detail="KinetiQ LEDs not configured")
    ...
    try:
        ...
        logger.error(f"Failed to set KinetiQ LED brightness: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def dw_leds_color(request: dict):
    ...
    try:
        ...
        logger.error(f"Failed to set KinetiQ LED color: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def dw_leds_colors(request: dict):
    ...
    try:
        ...
        logger.error(f"Failed to set KinetiQ LED colors: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def dw_leds_effects():
    ...
    try:
        ...
        logger.error(f"Failed to get KinetiQ LED effects: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def dw_leds_palettes():
    ...
    try:
        ...
        logger.error(f"Failed to get KinetiQ LED palettes: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def dw_leds_effect(request: dict):
    ...
    try:
        ...
        logger.error(f"Failed to set KinetiQ LED effect: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def dw_leds_palette(request: dict):
    ...
    try:
        ...
        logger.error(f"Failed to set KinetiQ LED palette: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def dw_leds_speed(request: dict):
    ...
    try:
        ...
        logger.error(f"Failed to set KinetiQ LED speed: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def dw_leds_intensity(request: dict):
    ...
    try:
        ...
        logger.error(f"Failed to set KinetiQ LED intensity: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def dw_leds_save_effect_settings(request: dict):
    ...
    state.save()
    logger.info(f"KinetiQ LED {effect_type} effect settings saved: {settings}")
...
async def dw_leds_clear_effect_settings(request: dict):
    ...
    state.save()
    logger.info(f"KinetiQ LED {effect_type} effect settings cleared")
...
async def dw_leds_set_idle_timeout(request: dict):
    ...
    state.save()
    logger.info(f"KinetiQ LED idle timeout configured: enabled={enabled}, minutes={minutes}")
...
async def trigger_update():
    """Trigger software update by running `kinetiq update` as a detached process.
...
async def restart_system():
    """Restart the KinetiQ Motion service via systemctl."""
    try:
        ...
        def delayed_restart():
            time.sleep(2)  # Give time for response to be sent
            try:
                subprocess.run(["/usr/bin/sudo", "/usr/bin/systemctl", "restart", "kinetiq-motion"], check=True)
...
# This route is deliberately registered after every API endpoint so it cannot
# intercept /api/* or other backend routes.
@app.get("/{frontend_path:path}", include_in_schema=False)
async def oryn_frontend_fallback(frontend_path: str):
