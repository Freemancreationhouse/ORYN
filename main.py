...
async def lifespan(app: FastAPI):
    # Startup
    logger.info("Starting Studio Kinematics KinetiQ Motion...")
    ...
    # Shutdown
    logger.info("Shutting down Studio Kinematics KinetiQ Motion...")

app = FastAPI(lifespan=lifespan)

# Add CORS middleware to allow cross-origin requests from other KinetiQ Motion frontends
...
async def pattern_generator_preview(file: UploadFile = File(...), threshold: int = Form(128), invert: bool = Form(False), fit: float = Form(0.94)):
    ...
    try:
        ...
        points,stats=await asyncio.to_thread(convert_upload_to_thr,Path(source),threshold,invert,fit)
